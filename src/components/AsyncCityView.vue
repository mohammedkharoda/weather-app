<template>
  <div class="flex flex-col flex-1 items-center">
    ...
    >
      <p>
        You are currently preview the city,click that "+" icon to start the
        tracking
      </p>
    </div>
    <!-- Weather Overview -->
...
const openWeather = import.meta.env.VITE_OPENWEATHER_KEY;
const getWeatherData = async () => {
  try {
    const weatherData = await axios.get(
      `https://api.openweathermap.org/data/3.0/onecall?lat=${route.query.lat}&lon=${route.query.lng}&exclude={part}&appid=${openWeather}&units=metric`
    );
